...
def getMissionCMD(mission_file, output_file, image_folder, reset, params):
    mission_path = f'{globals.cgi_path}/Missions/{mission_file}'
    
    cmd = ['python3', mission_path, output_file, image_folder, reset]
    cmd += params

    print(f'<br><br><br>{cmd}')
...
def startMission(mission_file, output_file, image_folder, reset, params):
    output_path = f'{globals.htdocs_path}/{output_file}'

    stopPrevMission(output_path)

    cmd = getMissionCMD(mission_file, output_file, image_folder, reset, params)
